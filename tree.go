...
// Makes a case-insensitive lookup of the given path and tries to find a callbackr.
...
walk: // Outer loop for walking the tree
	for len(path) >= npLen && (npLen == 0 || strings.EqualFold(path[1:npLen], n.path[1:])) {
		...
		if len(path) > 0 {
			...
			if !n.wildChild {
				...
				if fixTrailingSlash && path == "/" && n.callback != nil {
					return ciPath
				}
				return nil
			}
			...
			if fixTrailingSlash {
				for i, c := range []byte(n.indices) {
					if c == '/' {
						n = n.children[i]
						if (len(n.path) == 1 && n.callback != nil) ||
							(n.nType == catchAll && n.children[0].callback != nil) {
							return append(ciPath, '/')
						}
						return nil
	...
	if fixTrailingSlash {
		...
		if len(path)+1 == npLen && n.path[len(path)] == '/' &&
			strings.EqualFold(path[1:], n.path[1:len(path)]) && n.callback != nil {
